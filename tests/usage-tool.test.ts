...
describe('submit_daily_report 중복 제출 방지 (F-08)', () => {
  ...
  beforeEach(() => {
    ...
    rawDb.prepare(`INSERT INTO usage_logs (email, model, input_tokens, output_tokens, recorded_at)
                   VALUES ('test@example.com', 'sonnet', 100, 50, '2026-05-12 10:00:00')`).run();
  });

  it('처음 제출은 성공한다', async () => {
    const r = await handleUsageTool('submit_daily_report', { date: '2026-05-12' }, store);
    expect(r.isError).toBeUndefined();
    expect((r.content[0] as { text: string }).text).toContain('제출 성공');
  });

  it('같은 날짜 두 번째 제출은 거부된다', async () => {
    await handleUsageTool('submit_daily_report', { date: '2026-05-12' }, store);
    const r = await handleUsageTool('submit_daily_report', { date: '2026-05-12' }, store);
    expect(r.isError).toBe(true);
  ...
  it('force: true 로 강제 재제출이 가능하다', async () => {
    await handleUsageTool('submit_daily_report', { date: '2026-05-12' }, store);
    const r = await handleUsageTool('submit_daily_report', { date: '2026-05-12', force: true }, store);
    expect(r.isError).toBeUndefined();
  });

  it('데이터 없으면 안내 메시지 반환', async () => {
    const r = await handleUsageTool('submit_daily_report', { date: '2099-01-01' }, store);
    expect(r.isError).toBeUndefined();
